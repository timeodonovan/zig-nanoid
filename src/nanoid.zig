...
pub fn computeMask(alphabet_len: u8) u8
{
    ...
    const mask: u32 = (@as(u32, 2) << (31 - clz)) - 1;
    const result = @as(u8, @truncate(mask));
    return result;
}
...
pub fn computeRngStepBufferLength(id_len: usize, alphabet_len: u8) usize
{
    ...
    const mask_f = @as(f64, @floatFromInt(computeMask(alphabet_len)));
    const id_len_f = @as(f64, @floatFromInt(id_len));
    const alphabet_size_f = @as(f64, @floatFromInt(alphabet_len));
...
pub fn computeSufficientRngStepBufferLengthFor(max_id_len: usize) usize
{
    ...
    {
        const alphabet_len = @as(u8, @truncate(i));
...
pub fn generateEx(rng: std.Random, alphabet: []const u8, result_buffer: []u8, step_buffer: []u8) []u8
{
    ...
    const alphabet_len = @as(u8, @truncate(alphabet.len));
...
test "flat distribution of generated ids"
{
    ...
    {
        const value_f = @as(f64, @floatFromInt(value));
        const alphabet_len_f = @as(f64, @floatFromInt(alphabets.default.len));
        const count_f = @as(f64, @floatFromInt(number_of_ids_to_generate));
        const id_len_f = @as(f64, @floatFromInt(default_id_len));
